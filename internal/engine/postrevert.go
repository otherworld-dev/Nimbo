...
// RestoreInsteadOfDelete rewrites every "delete it on the server" action into a
// download, for the first sync pass after a folder leaves virtual-files mode.
//
...
func RestoreInsteadOfDelete(actions []Action, remote map[string]RemoteState) (out []Action, restored []string) {
	for _, a := range actions {
		...
		r, ok := remote[a.Path]
		if !ok {
			out = append(out, a) // nothing to restore from; leave it alone
			continue
		}
		if r.IsDir {
